...
const removeMealFromWeekSchema = z.object({
    mealId: z.string().describe("The ID of the meal to remove from the weekly selection"),
});

export const RemoveMealFromWeekTool = new DynamicStructuredTool({
  name: "remove_meal_from_week",
  description: "Removes a meal from the current weekly selection. The meal will remain in your deck and can be added back later.",
  schema: removeMealFromWeekSchema,
  func: async (input: z.infer<typeof removeMealFromWeekSchema>): Promise<string> => {
