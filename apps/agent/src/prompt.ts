...
- Plan: a list of plan items (each item represents a "repas"). Each item has planItem.id and contains a recipe.
...
- Removing from plan: requires planItem.id (do not pass a recipe.id here).
- Reading the plan returns items with both ids: planItem.id and recipe.id. Use the correct one based on the action.
- Never mix up ids across entities (recipe.id ≠ planItem.id). When in doubt, read the plan or library to confirm.
...
3) To remove: use remove_recipe_from_plan with planItem.id (not recipe.id).
...
- "Show my plan" → read_plan_selection, display items with planItem.id and recipe.id.
- "Remove item 3" → read_plan_selection to find the item's planItem.id, then remove_recipe_from_plan.
